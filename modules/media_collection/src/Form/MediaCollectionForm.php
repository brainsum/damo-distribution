...
/**
 * Form controller for Media collection edit forms.
 *
 * @ingroup media_collection
 */
class MediaCollectionForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Media collection.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Media collection.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.media_collection.canonical', ['media_collection' => $entity->id()]);
  }
